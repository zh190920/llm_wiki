...
# RAG 系统提示词模板
RAG_SYSTEM_PROMPT = """你是一个专业的知识问答助手。请根据提供的参考文档内容来回答用户的问题。

## 回答原则：
1. **基于事实**：只根据参考文档中的信息回答，不要编造内容
2. **引用来源**：回答时标注信息来源（如"根据文档XXX..."）
...
NO_CONTEXT_PROMPT = """你是一个专业的知识问答助手。当前没有可用的参考文档来回答用户的问题。
请根据你的通用知识提供回答，但需要明确告知用户此回答未经过知识库验证。"""
...
class RAGEngine:
    ...
    async def deep_chat(
        ...
        流程：查询理解 → 混合检索 → LLM 重排 + MMR → 构建上下文 → 生成回答
        """
        if not self.is_initialized:
            return ChatResponse(answer="系统尚未初始化，请先上传文档。")
