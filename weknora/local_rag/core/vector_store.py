...
class VectorStore:
    ...
    async def delete_by_doc_id(self, doc_id: str) -> int:
        """删除指定文档的所有块，从 FAISS 索引中重建（保留其余块的嵌入）"""
        async with self._lock:
            # 找到需要保留的块和需要删除的块索引
            kept_chunks = []
            kept_embeddings = []
            ...
            if removed_count > 0:
                ...
                # 重建 FAISS 索引，使用保留块的嵌入向量
                self._init_index()
                self._id_map.clear()

                if kept_embeddings:
                    vectors = np.array(kept_embeddings, dtype=np.float32)
                    # 重新归一化（reconstruct 可能损失精度）
                    faiss.normalize_L2(vectors)
                    start_idx = self._index.ntotal
                    self._index.add(vectors)
                ...
                # 重建 BM25 索引
                self._rebuild_bm25()
                logger.info(f"删除文档 {doc_id} 的 {removed_count} 个块，索引已重建，保留 {len(kept_chunks)} 个块")
